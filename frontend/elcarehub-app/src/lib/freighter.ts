...
export async function isFreighterInstalled(): Promise<boolean> {
  ...
  if ((window as any).starlight || (window as any).freighter) return true;
  try {
    const { isConnected } = await getFreighterApi();
...
export async function connectFreighter(): Promise<FreighterAccount> {
  const { setAllowed, getPublicKey, getNetworkDetails } = await getFreighterApi();
...
export async function signWithFreighter(
  ...
  const { signTransaction } = await getFreighterApi();
...
export async function getConnectedPublicKey(): Promise<string | null> {
  try {
    ...
    const { getPublicKey } = await getFreighterApi();
